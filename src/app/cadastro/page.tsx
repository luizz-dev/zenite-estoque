...
export default function CadastrarItemPage() {
  ...
  // categorias já cadastradas no estoque, sem duplicar por causa de maiúsculas/minúsculas
  const categoriasExistentes = Array.from(
    new Map(produtos.map((p) => [p.categoria.trim().toLowerCase(), p.categoria.trim()])).values()
  ...
  const salvar = async () => {
    ...
    try {
      // se já existe uma categoria igual (ignorando maiúsculas/minúsculas e espaços),
      // usa a grafia já cadastrada — evita duplicar "Camisas" e "camisas" no estoque.
      const categoriaExistente = categoriasExistentes.find((c) => c.toLowerCase() === form.categoria.trim().toLowerCase());
      const categoriaFinal = categoriaExistente || form.categoria.trim();

      await criarProduto({
        nome: form.nome, sku: form.sku, categoria: categoriaFinal || "Outros", unidade: form.unidade,
      ...
      <div className="zn-page-enter">
        ...
        <div style={{ maxWidth: 580, margin: "0 auto" }} className="zn-modal-enter">
          <Card style={{ textAlign: "center", padding: "52px 32px" }}>
            ...
            <p style={{ color: C.textSec, fontSize: 13.5, lineHeight: 1.7, margin: "0 0 28px" }}>
        ...
        {STEPS.map((s, i) => {
          ...
          return (
            <div key={s} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: i === 0 ? "flex-start" : i === STEPS.length - 1 ? "flex-end" : "center" }}>
              ...
              <p style={{ color: active ? C.purpleText : done ? C.cyanText : C.textMuted, fontSize: 11, fontWeight: 500, margin: "6px 0 0", textAlign: "center" }}>{s}</p>
            </div>
          );
        })}
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 300px", gap: 16, alignItems: "start" }}>
        <Card>
          <div style={{ marginBottom: 20 }}>
            ...
            <p style={{ color: C.textMuted, fontSize: 12.5, margin: 0 }}>{STEP_SUBS[step - 1]}</p>
          </div>
            ...
            {step === 1 && (
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                ...
                <div>
                  ...
                  <p style={{ color: C.textMuted, fontSize: 11, margin: "5px 0 0" }}>Identificador único do produto</p>
                </div>
                <div>
                  ...
                  {categoriasExistentes.length > 0 && (
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginTop: 8 }}>
                      {categoriasExistentes.map((c) => {
                        const active = form.categoria.trim().toLowerCase() === c.toLowerCase();
                        return (
                          <button key={c} type="button" onClick={() => set("categoria", c)} style={{
                            padding: "5px 11px", borderRadius: 999, border: `1px solid ${active ? C.purple1 : C.border}`, cursor: "pointer",
                            fontSize: 11.5, fontWeight: 500, background: active ? "rgba(72,55,232,0.18)" : "rgba(255,255,255,0.02)", color: active ? C.purpleText : C.textMuted,
                          }}>{c}</button>
                        );
                      })}
                    </div>
                    ...
                    {UNIDADES.map(([v, l]) => {
                      ...
                      return (
                        <button key={v} onClick={() => set("unidade", v)} style={{
                          ...
                          fontSize: 12.5, fontWeight: 500, background: active ? "rgba(72,55,232,0.2)" : "rgba(255,255,255,0.02)", color: active ? C.purpleText : C.textSec,
                  ...
                  <div style={{ gridColumn: "1/-1", border: `1px solid ${C.border}`, background: "rgba(255,255,255,0.02)", borderRadius: 12, padding: 16, display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 12 }}>
                    {[
                      ...
                      <div key={l as string} style={{ padding: "12px 14px", borderRadius: 10, border: `1px solid ${C.border}`, background: "rgba(255,255,255,0.02)" }}>
                        <p style={{ color: C.textMuted, fontSize: 11, margin: "0 0 4px" }}>{l}</p>
            ...
            {step === 3 && (
              <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
                  ...
                  <div>
                    <FieldLabel>Código NCM <span style={{ color: C.red }}>*</span> <span style={{ color: C.cyan, fontSize: 11 }}>← obrigatório para NF-e</span></FieldLabel>
                    <Input value={form.ncm} onChange={(e) => set("ncm", e.target.value)} placeholder="Ex: 6104.4200" />
                    <p style={{ color: C.textMuted, fontSize: 11, margin: "5px 0 0" }}>Nomenclatura Comum do Mercosul — cada tipo de peça pode ter um NCM diferente</p>
                  </div>
                </div>
                <div style={{ borderRadius: 12, border: "1px solid rgba(245,124,0,0.22)", background: "rgba(245,124,0,0.06)", padding: "12px 14px", display: "flex", gap: 10 }}>
                  ...
                  <div>
                    <p style={{ color: C.amber, fontSize: 12.5, fontWeight: 600, margin: "0 0 3px" }}>NCM incorreto causa rejeição da NF-e pela SEFAZ</p>
                    <p style={{ color: C.textMuted, fontSize: 11.5, margin: 0, lineHeight: 1.5 }}>Verifique o código na tabela NCM disponível no portal da Receita Federal antes de cadastrar. Definir aqui evita retrabalho na hora de emitir a nota.</p>
                  </div>
                </div>
                <div>
                  ...
                  <textarea value={form.descricao} onChange={(e) => set("descricao", e.target.value)}
                    ...
                    style={{
                      ...
                      fontSize: 13.5, color: C.white, outline: "none", boxSizing: "border-box", fontFamily: "inherit",
            ...
            {step === 4 && (
              <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                <div style={{ borderRadius: 12, border: "1px solid rgba(72,55,232,0.25)", background: "rgba(72,55,232,0.06)", padding: 18 }}>
                  <p style={{ color: C.purpleText, fontSize: 12, fontWeight: 600, margin: "0 0 14px", textTransform: "uppercase", letterSpacing: "0.06em" }}>Resumo do Cadastro</p>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                    {[
                      ...
                      <div key={k}><p style={{ color: C.textMuted, fontSize: 11, margin: 0 }}>{k}</p><p style={{ color: C.white, fontSize: 13, fontWeight: 500, margin: "2px 0 0" }}>{v}</p></div>
                    ))}
                  </div>
                </div>
                {form.descricao && (
                  <div style={{ borderRadius: 12, border: `1px solid ${C.border}`, background: "rgba(255,255,255,0.02)", padding: "12px 14px" }}>
                    <p style={{ color: C.textMuted, fontSize: 11, margin: "0 0 4px" }}>Descrição Técnica</p>
                    <p style={{ color: C.textSec, fontSize: 12.5, margin: 0, lineHeight: 1.6 }}>{form.descricao}</p>
                  </div>
                )}
                <div style={{ borderRadius: 12, border: "1px solid rgba(74,222,128,0.2)", background: "rgba(74,222,128,0.05)", padding: "12px 14px", display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  <p style={{ color: C.green, fontSize: 12.5, fontWeight: 600, margin: 0 }}>Pronto para cadastrar — todos os campos preenchidos</p>
                </div>
              </div>
            )}
          </div>
          ...
          {erro && (
            <div style={{ borderRadius: 12, border: "1px solid rgba(248,113,113,0.3)", background: "rgba(248,113,113,0.08)", padding: "11px 14px", marginTop: 14, color: C.red, fontSize: 12.5, display: "flex", alignItems: "center", gap: 8 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          <Card>
            <p style={{ color: C.purpleText, fontSize: 11.5, fontWeight: 600, margin: "0 0 14px", textTransform: "uppercase", letterSpacing: "0.06em" }}>Prévia do Produto</p>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
              ...
              <div>
                ...
                <p style={{ color: C.textMuted, fontSize: 11.5, margin: "2px 0 0" }}>{form.sku || "SKU"} · {form.categoria || "Categoria"}</p>
              </div>
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              <div style={{ display: "flex", justifyContent: "space-between" }}><span style={{ color: C.textMuted, fontSize: 12 }}>Preço de Venda</span><span style={{ color: C.white, fontSize: 12.5, fontWeight: 500 }}>{form.precoVenda ? brl(+form.precoVenda) : "—"}</span></div>
              <div style={{ display: "flex", justifyContent: "space-between" }}><span style={{ color: C.textMuted, fontSize: 12 }}>Qtd. Inicial</span><span style={{ color: C.textSec, fontSize: 12.5, fontWeight: 500 }}>{form.quantidade || "0"} un.</span></div>
              <div style={{ display: "flex", justifyContent: "space-between" }}><span style={{ color: C.textMuted, fontSize: 12 }}>Fornecedor</span><span style={{ color: C.textSec, fontSize: 12.5, fontWeight: 500 }}>{form.fornecedor || "—"}</span></div>
              <div style={{ display: "flex", justifyContent: "space-between" }}><span style={{ color: C.textMuted, fontSize: 12 }}>NCM</span><span style={{ color: C.textSec, fontSize: 12.5, fontWeight: 500 }}>{form.ncm || "—"}</span></div>
              {margem && (
                <>
                  <div style={{ height: 1, background: C.border, margin: "2px 0" }} />
                  <div style={{ display: "flex", justifyContent: "space-between" }}>
                    <span style={{ color: C.textMuted, fontSize: 12 }}>Margem Bruta</span>
                    <span style={{ color: +margem >= 40 ? C.green : +margem >= 25 ? C.amber : C.red, fontSize: 13.5, fontWeight: 700 }}>{margem}%</span>
                  </div>
                </>
              )}
            </div>
